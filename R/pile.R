...
has_log <- function(pile, fingerprint, namespace = "index") {
  pile$st$exists(fingerprint, namespace = namespace)
}

pile_get_record <- function(pile, fingerprint, namespace = "index") {
  if (!has_log(pile, fingerprint, namespace)) {
    return(NULL)
  }
  pile$st$get(fingerprint, namespace = namespace)
}
...
#' Get log from pile
get_log <- function(pile, fingerprint, namespace = "index") {
  rec <- pile_get_record(pile, fingerprint, namespace)
  if (is.null(rec)) {
    if (namespace == "index") {
  ...
  if (namespace == "index") {
    ...
    df <- as.data.frame(read_log(path, rec$row_selector), stringsAsFactors = FALSE)
  ...
  if (identical(rec$type, "parquet")) {
    path <- record_path(pile, fingerprint, rec, namespace)
    if (is.null(path)) return(NULL)
    return(as.data.frame(read_log(path, rec$row_selector), stringsAsFactors = FALSE))
  }
...
index_record <- function(pile, fingerprint, value, meta, attempts) {
  ...
  if (is_failure) {
    ...
    rel_file <- write_parquet(
      ...
      model = model_id,
      bucket = bucket,
      filename = sprintf("log-%s-0.parquet", fingerprint)
    )
...
projection_record <- function(pile, fingerprint, value, namespace) {
...
#' Put value into pile
put_log <- function(pile, fingerprint, value, meta = NULL, namespace = "index") {
  existing <- pile_get_record(pile, fingerprint, namespace)
  if (!is.null(existing) && identical(existing$status, "done")) return(invisible(NULL))
  attempts <- (existing$attempts %||% 0L) + 1L

  rec <- if (namespace == "index")
    index_record(pile, fingerprint, value, meta, attempts)
  else
    projection_record(pile, fingerprint, value, namespace)

  pile$st$set(fingerprint, rec, namespace = namespace)
  invisible(NULL)
}

#' Read a parquet file, optionally selecting a single run by fingerprint.
read_log <- function(path, row_selector = NULL) {
  if (is.null(row_selector)) {
    arrow::read_parquet(path)
  } else {
    ds <- arrow::open_dataset(path, format = "parquet")
    dplyr::collect(dplyr::filter(ds, fingerprint == !!row_selector$fingerprint))
  }
}
...
relink_records <- function(pile, namespace, fingerprints, rel_path, create = FALSE) {
  records <- pile$st$mget(fingerprints, namespace = namespace)
  valid_idx <- if (create) {
    rep(TRUE, length(fingerprints))
  } else {
  ...
  valid_fps <- fingerprints[valid_idx]
  ...
  for (i in seq_along(valid_records)) {
    ...
    valid_records[[i]]$row_selector <- list(fingerprint = valid_fps[i])
  }
...
# Merge every per-log parquet under one partition into a single sorted file,
...
compact_partition <- function(pile, prefix, key, namespace, sort_cols) {
  ...
  log_files <- fs::dir_ls(dir, recurse = TRUE, regexp = "log-.*\\.parquet$")
  if (length(log_files) == 0L) return(invisible(NULL))
  
  ds <- arrow::open_dataset(log_files, format = "parquet")
  ...
  fingerprints <- unique(df$fingerprint)
  relink_records(pile, namespace, fingerprints, rel_path, create = FALSE)
...
compact_pile <- function(pile, model) {
  compact_partition(pile, "raw", sprintf("model=%s", model), "index",
                    c("fingerprint", "strategy_id", "cohort_id", "t"))
}

#' Compact Projections
#'
#' @param pile A logpile_pile object.
#' @param id Projection version string.
#' @export
compact_projections <- function(pile, id) {
  compact_partition(pile, "projections", sprintf("projection=%s", id),
                    id,
                    c("fingerprint", "strategy_id", "cohort_id", "t", "bin"))
}
